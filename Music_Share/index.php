...
<head>
    ...
    <link rel="stylesheet" href="Diseños/index(Home)_Diseño.css">
...
<body>
<!-- ------------------------------------------------------------------------- Promocionales de la pagina -->
    <div class="Promocional">
        <?php
        include 'Funciones/conecta.php';
        $con = conecta();

        $sql = "SELECT * FROM promociones WHERE Status = 1 AND Eliminado = 0 ORDER BY RAND() LIMIT 1"; // esto hace que se muestre aleatorio
        $result = $con->query($sql);

        if ($result->num_rows > 0) {
            $rowPromo = $result->fetch_assoc();
            echo "<img class='Imagen-Promo' src='Admin/Archivos/Promociones/" . $rowPromo["Archivo"] . "' alt='" . $rowPromo["Nombre"] . "'>";
        } else {
            echo "<img clas='Imagen-Promo' src='Diseños/imagenes-Diseño/Music-Share.png >"; // Imagen que se muestra en caso de no tener promociones
        }
